...
quasiELst = utils.cmplxZeros( ( V1_lst.shape ) + (V2_lst.shape) + (N,) )
eVecLst = utils.cmplxZeros( ( V1_lst.shape ) + (V2_lst.shape) + (N,) + (N,) )
...
if filenamesLst != None:
	np.save( filenamesLst[0], [V1_lst, V2_lst], delimiter=',',fmt='%f' )
	np.save( filenamesLst[1], np.real( np.round( chernLst, 2 ) ), delimiter=',',fmt='%f' )
	np.save( filenamesLst[2], quasiELst, delimiter=',',fmt='%f' )